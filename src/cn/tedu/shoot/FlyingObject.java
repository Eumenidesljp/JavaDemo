...
public abstract class FlyingObject {
	...
	public static final int DEAD = 1;   //死了的(还未删除)
	...
	public static BufferedImage loadImage(String fileName) {
		try {
			BufferedImage img = ImageIO.read(FlyingObject.class.getResource(fileName)); //同包内读图片
			return img;
		}catch(Exception e) {
			e.printStackTrace();
	...
	public boolean isLife() {
		return state==LIFE; //当前状态为LIFE表示活着，返回true
	}
	/** 判断是否是死了的 */
	public boolean isDead() {
		return state==DEAD; //当前状态为DEAD表示死了，返回true
	}
	/** 判断是否是删除的 */
	public boolean isRemove() {
		return state==REMOVE; //当前状态为REMOVE表示删除，返回true
	}
